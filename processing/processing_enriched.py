...
def generate_table_enriched(table_name:str) -> bool:
    ...
    if table_name == 'flight_airport_departures_enriched':
        ...
        substitions.data = substitions.data.with_columns(
            pl.col("minute_absolute").rank('dense').over(['game_uuid','player_uuid','type']).alias("minute_absolute_rank")
        )
        substitions_in = substitions.custom_select(["game_uuid","player_uuid","minute_absolute_rank","type","point_diff"])
        substitions_in.data = substitions_in.data.filter(pl.col('type') == 'IN_TYPE')
        substitions_out = substitions.custom_select(["game_uuid","player_uuid","minute_absolute_rank","type","point_diff"])
        ...
        playerplusminus_enriched = CustomDF('playerplusminus_enriched',initial_df=substitions.data)
        ...
        playerstatssummary_enriched = CustomDF('playerstatssummary_enriched',initial_df=player_stats.data)
        ...
        playergameshotsdata_df.data = playergameshotsdata_df.data.filter(pl.col('shot_type') == '2PT').unique()
        playergameshotsdata_df = playergameshotsdata_df.custom_groupby(
            [
                "player_uuid","season"
            ],
            pl.struct([pl.col("xnormalize"), pl.col("ynormalize")]).alias("twopoint_locations")
        )
        ...
        player2ptsummary_enriched = CustomDF('player2ptsummary_enriched',initial_df=playergameshotsdata_df.data)
        ...
        playergameshotsdata_df.data = playergameshotsdata_df.data.filter(pl.col('shot_type') == '3PT').unique()
        playergameshotsdata_df = playergameshotsdata_df.custom_groupby(
            [
                "player_uuid","season"
            ],
            pl.struct([pl.col("xnormalize"), pl.col("ynormalize")]).alias("threepoint_locations")
        )
        ...
        player3ptsummary_enriched = CustomDF('player3ptsummary_enriched',initial_df=playergameshotsdata_df.data)
        ...
        player_data = player_data.custom_join(
        ...
        player_analytics_enriched = CustomDF('playeranalytics_enriched',initial_df=player_analytics.data)
        ...
        teamstatssummary_enriched = CustomDF('teamstatssummary_enriched',initial_df=teamstatssummary.data)
        ...
        opponents_stats.data = opponents_stats.data.filter(pl.col("team_uuid_right") != pl.col("team_uuid"))
        ...
        opponentsstatssummary_enriched = CustomDF('opponentsstatssummary_enriched',initial_df=opponents_stats.data)
        ...
        opponents_stats.data = opponents_stats.data.with_columns(
            ((pl.col("points") - pl.col("average_points"))/pl.col("average_points")).alias("points_diff"),
            ((pl.col("two_made") - pl.col("avg_two_made"))/pl.col("avg_two_made")).alias("2pt_diff"),
            ((pl.col("three_made") - pl.col("avg_three_made"))/pl.col("avg_three_made")).alias("3pt_diff"),
            ((pl.col("ft_made") - pl.col("avg_ft_made"))/pl.col("avg_ft_made")).alias("ftmade_diff"),
            ((pl.col("ft_attempted") - pl.col("avg_ft_attempted"))/pl.col("avg_ft_attempted")).alias(
                "ftattempted_diff"
            ),
        )
        ...
        opponentstrendssummary_enriched= CustomDF('opponentstrendssummary_enriched',initial_df=opponents_stats.data)
        ...
        team_analytics = teamdata.custom_join(
