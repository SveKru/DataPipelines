...
def generate_table_raw(table_name:str) -> bool:
    ...
    if table_name == 'flights_raw':
        ...
        gamedata_landingzone.data = gamedata_landingzone.data.with_columns(pl.col('file_name').str.replace(".json","").alias('idMatchIntern'))
        gamedata_landingzone.data = gamedata_landingzone.data.with_columns(pl.col('time').str.strptime(pl.Datetime, "%b %d, %Y %I:%M:%S %p",exact=False).alias('time'))
        ...
        teamdata_landingzone = teamdata_landingzone.custom_select(['teamIdIntern','teamIdExtern','name','shortName'])
        ...
        for column in ["starting","captain"]:
            playergamedata_landingzone.data = playergamedata_landingzone.data.with_columns(pl.col(column).str.replace("true","1").str.replace("false","0").cast(pl.Int16).cast(pl.Boolean).alias(column))
        playergamedata_landingzone.convert_data_types(["dorsal","timePlayed"],pl.Int16)

        playergamedata_landingzone.data = playergamedata_landingzone.data.with_columns(pl.col('file_name').str.replace(".json","").alias('idMatchIntern'))
        playergamedata_landingzone.data = playergamedata_landingzone.data.with_columns(pl.col('uuid').alias('playerUuid'))
        playergamedata_landingzone = playergamedata_landingzone.custom_select(["actorId","playerUuid","teamId","name","playerInternTeam","dorsal","starting","captain","timePlayed",'idMatchIntern'])
        playergamedata_raw = CustomDF('playergamedata_raw',initial_df=playergamedata_landingzone.data)
        playergamedata_raw.write_table()

    elif table_name == 'playersubstitionsgamedata_raw':
        playersubstitionsgamedata_landingzone = CustomDF('playersubstitionsgamedata_landingzone')

        playersubstitionsgamedata_landingzone.data = playersubstitionsgamedata_landingzone.data.explode('inOutsList').unnest('inOutsList')
        playersubstitionsgamedata_landingzone.data = playersubstitionsgamedata_landingzone.data.with_columns(pl.col('file_name').str.replace(".json","").alias('idMatchIntern'))
        playersubstitionsgamedata_landingzone.data = playersubstitionsgamedata_landingzone.data.with_columns(pl.col('uuid').alias('playerUuid')).unique()
        playersubstitionsgamedata_landingzone = playersubstitionsgamedata_landingzone.custom_select(["playerUuid","actorId","teamId","type","minuteAbsolut","pointDiff","idMatchIntern"])

        playersubstitionsgamedata_raw = CustomDF('playersubstitionsgamedata_raw',initial_df=playersubstitionsgamedata_landingzone.data)
        ...
        playergamestatsdata_landingzone.data = playergamestatsdata_landingzone.data.unnest('data')
        playergamestatsdata_landingzone.data = playergamestatsdata_landingzone.data.with_columns(pl.col('file_name').str.replace(".json","").alias('idMatchIntern'))
        playergamestatsdata_landingzone.data = playergamestatsdata_landingzone.data.with_columns(pl.col('uuid').alias('playerUuid'))
        ...
        teamgamestatsdata_landingzone.data = teamgamestatsdata_landingzone.data.unnest('data')
        teamgamestatsdata_landingzone.data = teamgamestatsdata_landingzone.data.with_columns(pl.col('file_name').str.replace(".json","").alias('idMatchIntern'))
        teamgamestatsdata_landingzone.data = teamgamestatsdata_landingzone.data.with_columns(pl.col('teamIdExtern').alias('team_uuid'))
        ...
        twopoints_made = playergameshotsdata_landingzone.custom_select(['player_uuid','game_uuid','shootingOfTwoSuccessfulPoint'])
