...
class DataReader:
    ...
    def __init__(
        ...
        self._data_path = get_data_location(self._schema, self._partition_name)
    ...
    def read_source(self) -> DataFrame:
        ...
        if self._schema["container"] not in ["landingzone", "monitoring"]:
            df = create_map_column(
                df, "_".join([self._schema["location"], self._schema["container"]])
            )
    ...
    def read_csv_file(self):
        df = (
            self._spark_session.read.format("csv")
    ...
    def read_parquet_file(self):
        df = (
            self._spark_session.read.format("parquet")
            .schema(self._schema["columns"])
            .option("header", True)
            .load(self._data_path)
        )
    ...
    def read_delta_table(self):
        df = self._spark_session.read.format("delta").load(self._data_path)
