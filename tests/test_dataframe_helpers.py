...
class TestDataFrameHelpers:
    def test_create_map_column(self):
        df = pl.DataFrame({"RecordID": ["123", "456", "789"]})
        ...
        excepted = pl.DataFrame(
            {
                ...
                "shaValue": [
                    "83494463727554711",
                    "15276044730183553259",
                    "14156979967077821316",
                ],
            }
        )
    ...
    def test_get_data_location(self):
        ...
        expected = os.path.join(
            ...
            "my_container",
            "my_location",
            "my_partition=2023-01-01",
        )
        assert result == expected

        result_no_partition = get_data_location(schema)
        expected_no_partition = os.path.join(
            os.getcwd(), "data", "my_container", "my_location"
        )
        assert result_no_partition == expected_no_partition

    def test_clean_column_names(self):
        df = pl.DataFrame({"col-1": [1, 2, 3], "col(2)": [4, 5, 6], "col 3": [7, 8, 9]})
        result = clean_column_names(df)
        expected = pl.DataFrame(
            {"col_1": [1, 2, 3], "col2": [4, 5, 6], "col_3": [7, 8, 9]}
        )
        assert_frame_equal(result, expected)
    ...
    def test_only_deleted_apply_scd_type_2(self):
        incoming_data = pl.DataFrame(
            {"col1": ["a", "b"], "col2": ["1", "2"], "RecordID": ["123", "456"]}
        )
