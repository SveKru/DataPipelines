...
def test_check_value_within_list():
    dataframe = pl.DataFrame({"col1": ["a", "b", "c", "d", "e"]})
    kwargs = {"columns": ["col1"], "value_list": ["a", "b", "c"]}
...
def test_calculate_filled_values():
    dataframe = pl.DataFrame(
        {"col1": ["a", "b", None, "d", "e"], "col2": [1, 2, 3, None, 5]}
    )
...
def test_check_values_in_range():
    dataframe = pl.DataFrame({"col1": [1, 2, 3, 4, 5]})
    kwargs = {"columns": ["col1"], "range_start": 2, "range_end": 4}
    result = check_values_in_range(dataframe, **kwargs)
    assert result == 3


def test_check_values_unique():
    dataframe = pl.DataFrame({"col1": [1, 2, 2, 4, 5]})
    kwargs = {"columns": ["col1"]}
...
def test_check_values_format():
    dataframe = pl.DataFrame({"col1": ["abc", "def", "ghi", "123", "456"]})
    kwargs = {"columns": ["col1"], "format": r"^[a-z]+$"}
...
def test_check_values_consistent():
    dataframe = pl.DataFrame(
        {"col1": [1, 2, 3, 4, 5], "join_col": ["a", "b", "c", "d", "e"]}
    )
...
def test_check_expected_value_count():
    dataframe = pl.DataFrame(
        {"col1": ["a", "a", "b", "b", "b"], "RecordID": [1, 2, 3, 4, 5]}
    )
    kwargs = {"columns": ["col1"], "expected_count": 2}
...
def test_check_expected_distinct_value_count():
    ...
    kwargs = {"columns": ["col1"], "expected_count": 2, "distinct_columns": ["col2"]}
...
def test_column_sums_to_1():
    dataframe = pl.DataFrame(
        {"col1": ["a", "a", "b", "b", "b"], "value": [0.5, 0.5, 0.3, 0.3, 0.4]}
    )
    kwargs = {"columns": ["col1"], "sum_column": "value"}
...
def test_calculate_blocking_issues_values_in_range():
    dataframe = pl.DataFrame(
        {"col1": [1, 2, 3, 4, 5], "to_date": [date(2099, 12, 31)] * 5}
    )
    blocking_check_dict = [
        {
...
def test_calculate_blocking_issues_values_in_range_violation():
    dataframe = pl.DataFrame(
        {"col1": [1, 2, 3, 4, 6], "to_date": [date(2099, 12, 31)] * 5}
    )
    blocking_check_dict = [
        {
            "check": "values in range",
    ...
    with pytest.raises(
        ValueError, match="Blocking issue violation detected: values in range"
    ):
        calculate_blocking_issues(dataframe, blocking_check_dict)
...
def test_calculate_blocking_issues_values_are_unique():
    ...
    blocking_check_dict = [{"check": "values are unique", "columns": ["col1"]}]
    try:
        calculate_blocking_issues(dataframe, blocking_check_dict)
    except ValueError:
        pytest.fail("Unexpected ValueError raised")
...
def test_calculate_blocking_issues_values_are_unique_violation():
    dataframe = pl.DataFrame(
        {"col1": [1, 2, 2, 4, 5], "to_date": [date(2099, 12, 31)] * 5}
    )
    blocking_check_dict = [{"check": "values are unique", "columns": ["col1"]}]
    with pytest.raises(
        ValueError, match="Blocking issue violation detected: values are unique"
    ):
        calculate_blocking_issues(dataframe, blocking_check_dict)
...
def test_calculate_blocking_issues_values_have_format():
    ...
    blocking_check_dict = [
        {"check": "values have format", "columns": ["col1"], "format": r"^[a-z]+$"}
    ]
    try:
        calculate_blocking_issues(dataframe, blocking_check_dict)
    except ValueError:
...
def test_calculate_blocking_issues_values_have_format_violation():
    ...
    with pytest.raises(
        ValueError, match="Blocking issue violation detected: values have format"
    ):
        calculate_blocking_issues(dataframe, blocking_check_dict)
...
def test_calculate_signalling_issues_values_within_list():
    dataframe = pl.DataFrame(
        {"col1": ["a", "b", "c", "d", "e"], "col2": [1, 2, 3, None, 5]}
    )
...
def test_calculate_signalling_issues_values_in_range():
    dataframe = pl.DataFrame({"col1": [1, 2, 3, 4, 5], "col2": [1, 2, 3, None, 5]})
    signalling_check_dict = [
        {
            "check": "values in range",
            "columns": ["col1"],
...
def test_calculate_signalling_issues_values_are_unique():
    dataframe = pl.DataFrame({"col1": [1, 2, 2, 4, 5], "col2": [1, 2, 3, None, 5]})
    signalling_check_dict = [{"check": "values are unique", "columns": ["col1"]}]
    validation_df = pl.DataFrame(
        {
            "signalling_id": [None, None, None],
...
def test_calculate_signalling_issues_values_have_format():
    dataframe = pl.DataFrame(
        {"col1": ["abc", "def", "ghi", "123", "456"], "col2": [1, 2, 3, None, 5]}
    )
    signalling_check_dict = [
        {"check": "values have format", "columns": ["col1"], "format": r"^[a-z]+$"}
    ]
...
def test_calculate_signalling_issues_values_occur_as_expected():
    ...
    signalling_check_dict = [
        {"check": "values occur as expected", "columns": ["col1"], "expected_count": 2}
    ]
    validation_df = pl.DataFrame(
        {
            "signalling_id": [None, None, None],
...
def test_calculate_signalling_issues_values_sum_to_1():
    ...
    signalling_check_dict = [
        {"check": "values sum to 1", "columns": ["col1"], "sum_column": "value"}
    ]
    validation_df = pl.DataFrame(
        {
            ...
            "check_name": [
                ...
                "Check if values are filled",
                "Check if values are filled",
                'values in column "value" sum to 1',
            ],
            "total_count": [5, 5, 5, 5],
            "valid_count": [5, 5, 4, 5],
        }
...
def test_calculate_signalling_issues_distinct_values_occur_as_expected():
    dataframe = pl.DataFrame(
        {"col1": ["a", "a", "b", "b", "b"], "col2": [1, 2, 3, None, 5]}
    )
    signalling_check_dict = [
        {
    ...
    validation_df = pl.DataFrame(
        {
            ...
            "check_name": [
                "Check if values are filled",
                "Check if values are filled",
                "2 distinct values occur in column col2",
            ],
            "total_count": [5, 5, 5],
            "valid_count": [5, 4, 2],
        }
...
def test_record_has_expected_history():
    dataframe = pl.DataFrame({"RecordID": [1, 2, 3, 4, 5]})
    ...
    kwargs = {"expected_trace": expected_trace, "record_trace_table": mock_df}
