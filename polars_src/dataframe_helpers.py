...
def create_map_column(df: pl.DataFrame, dataframe_name: str) -> pl.DataFrame:
    ...
    """
    return (
        df.with_columns(pl.col("RecordID").str.split(" ").alias(dataframe_name))
        .with_columns(pl.struct(pl.col(dataframe_name)).alias(f"map_{dataframe_name}"))
        .drop(dataframe_name)
    )
...
def apply_scd_type_2(
    new_table: pl.DataFrame, existing_table: pl.DataFrame
) -> pl.DataFrame:
    """
    ...
    if [col for col in new_table.columns if col.startswith("map_")]:
        ...
        if not "signalling_id" in existing_table.columns:
            map_col = [col for col in new_table.schema if col.startswith("map_")][0]
    ...
    value_columns = [
        ...
        if col not in ["RecordID", "from_date", "to_date"] and col != map_col
    ]
    ...
    new_data_frame = (
        create_sha_values(new_table, value_columns)
        .with_columns(
            [
    ...
    closed_mask = (
        combined_df["shaValueOld"].is_not_null() & combined_df["shaValueNew"].is_null()
    )
    if combined_df.filter(closed_mask).height > 0:
        closed_records = (
            combined_df.filter(closed_mask)
    ...
    new_mask = (
        combined_df["shaValueOld"].is_null() & combined_df["shaValueNew"].is_not_null()
    )
    if combined_df.filter(new_mask).height > 0:
        new_records = (
            combined_df.filter(new_mask)
...
def assign_signalling_id(
    monitoring_values_df: pl.DataFrame, existing_monitoring_df: pl.DataFrame
) -> pl.DataFrame:
    """
    Assigns signalling IDs to monitoring values.
    """
    max_issue = (
        existing_monitoring_df.with_columns(
    ...
    existing_monitoring_df = existing_monitoring_df.select(
        [
            pl.col(c).alias(f"{c}_old")
            for c in [
                "signalling_id",
                "column_name",
                "check_name",
    ...
    non_existing_signalling_id = monitoring_values_intermediate.filter(
        pl.col("signalling_id_old").is_null()
    ).with_columns(
        [
            (pl.col("check_id").rank("ordinal").over("table_name") + max_issue)
            .cast(pl.Int64)
            .alias("signalling_id")
        ]
    )
    ...
    if existing_signalling_id.height == 0:
        ...
        monitoring_values_df = (
            pl.concat([existing_signalling_id, non_existing_signalling_id])
            .with_columns(
                pl.coalesce("signalling_id_old", "signalling_id").alias("signalling_id")
            )
            .select(
