...
def calculate_filled_values(dataframe: DataFrame) -> DataFrame:
    ...
    for c in df_cols:
        if dataframe.schema[c] == pl.String:
            ...
            invalid_count = dataframe.filter((pl.col(c).is_null())).height
    ...
    df = pl.DataFrame(results).with_columns(
        pl.lit(None).cast(pl.Int64).alias("signalling_id")
    )
    col_order = [
        "signalling_id",
        "check_id",
...
def check_expected_value_count(dataframe: DataFrame, **kwargs: dict) -> int:
    ...
    valid_count = dataframe.join(valid_rows, on=groupby_columns, how="inner").height
...
def record_has_expected_history(dataframe: DataFrame, **kwargs: dict):
    ...
    record_trace_table = record_trace_table.join(
        dataframe.select("RecordID"),
        left_on=["target_RecordID"],
        right_on=["RecordID"],
        how="inner",
    )
    record_trace_table = record_trace_table.group_by(
        "target_RecordID", "source_table_name"
    ).agg(pl.len().alias("amount_records"))

    expected_df = (
        pl.DataFrame(expected_trace)
        .join(record_trace_table.select("target_RecordID"), how="cross")
        .select(["target_RecordID", "source_table_name", "amount_records"])
    )

    invalid_count = (
        record_trace_table.join(
            expected_df,
            on=["target_RecordID", "source_table_name", "amount_records"],
            how="anti",
        )
        .select("target_RecordID")
        .unique()
        .height
    )
...
def calculate_blocking_issues(
    dataframe: DataFrame, blocking_check_dict: List[Dict[str, Any]]
) -> None:
    """
...
def calculate_signalling_issues(
    dataframe: DataFrame, signalling_check_dict: List[Dict[str, Any]]
) -> DataFrame:
    """
