...
def export_edges():
    ...
    target_traces = recrord_traces.filter(pl.col('source_RecordID').is_not_null()).group_by([
        'target_table_name',
        'target_RecordID'
    ]).agg(
        pl.col('source_RecordID').alias('upstream')
    ).rename({
    ...
    missing_sources = recrord_traces.filter(pl.col('source_table_name').str.ends_with('raw')).select([
    ...
    recrord_traces = pl.concat([target_traces.select(col_order),missing_sources.select(col_order)],how='vertical').filter(pl.col('id').is_not_null())
